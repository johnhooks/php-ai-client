...
class AnthropicTextGenerationModelTest extends TestCase
{
    ...
    public function testStreamGenerateTextResultThrowsException(): void
    {
        ...
        $this->expectExceptionMessage('Streaming is not yet implemented for Anthropic provider.');
    ...
    /**
     * Tests that candidateCount > 1 throws exception.
     *
     * @return void
     */
    public function testCandidateCountGreaterThanOneThrowsException(): void
    {
        $prompt = [new Message(MessageRoleEnum::user(), [new MessagePart('Hello')])];
        ...
        $this->expectExceptionMessage('Unsupported parameter "candidateCount" for Anthropic provider.');

        $model->generateTextResult($prompt);
    }

    /**
     * Tests that presencePenalty throws exception.
     *
     * @return void
     */
    public function testPresencePenaltyThrowsException(): void
    {
        $prompt = [new Message(MessageRoleEnum::user(), [new MessagePart('Hello')])];
        $modelConfig = ModelConfig::fromArray(['presencePenalty' => 0.5]);
        $model = $this->createModel($modelConfig);

        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('Unsupported parameter "presencePenalty" for Anthropic provider.');

        $model->generateTextResult($prompt);
    }

    /**
     * Tests that frequencyPenalty throws exception.
     *
     * @return void
     */
    public function testFrequencyPenaltyThrowsException(): void
    {
        $prompt = [new Message(MessageRoleEnum::user(), [new MessagePart('Hello')])];
        $modelConfig = ModelConfig::fromArray(['frequencyPenalty' => 0.5]);
        $model = $this->createModel($modelConfig);

        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('Unsupported parameter "frequencyPenalty" for Anthropic provider.');

        $model->generateTextResult($prompt);
    }

    /**
     * Tests that logprobs throws exception.
     *
     * @return void
     */
    public function testLogprobsThrowsException(): void
    {
        $prompt = [new Message(MessageRoleEnum::user(), [new MessagePart('Hello')])];
        ...
        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('Unsupported parameter "logprobs" for Anthropic provider.');
    ...
    /**
     * Tests that JSON output mode includes output_format parameter.
     *
     * @return void
     */
    public function testJsonOutputModeIncludesOutputFormat(): void
    {
        ...
        $this->assertEquals(['type' => 'json'], $requestData['output_format']);
    }
